...
class SuccessHud: UIView {
    
    var text = ""
    
    class func hud(inView view: UIView, animated: Bool) -> SuccessHud {
        let hudView = SuccessHud(frame: view.bounds)
    ...
    override func draw(_ rect: CGRect) {
        ...
        if let image = UIImage(named: "success")?.withTintColor(.systemGreen) {
            let imagePoint = CGPoint(
                ...
                y: center.y - round(image.size.height /
                                    2) - boxHeight / 8)
